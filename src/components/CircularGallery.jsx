...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    const isPortrait = this.screen.width < this.screen.height;
    this.scale = isPortrait
      ? this.screen.width / 800
      : this.screen.height / 1500;

    const baseHeight = isPortrait ? 600 : 900;
    const baseWidth = isPortrait ? 500 : 700;
...
class App {
      ...
      return new Media({
        ...
        onItemClick: () => {
          ...
          window.dispatchEvent(
            new CustomEvent("openPortfolioModal", { detail: data }),
          );
        },
      });
    });
  ...
  onTouchMove(e) {
    if (this.isDown) {
      ...
      // If vertical movement is significant and greater than horizontal, allow page scroll
      if (deltaY > deltaX && deltaY > 10) {
        this.isDown = false;
        return;
      }

      if (deltaX > 5) this.isDragging = true;
